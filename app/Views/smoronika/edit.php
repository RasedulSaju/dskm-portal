...
<div class="card" style="max-width: 800px; margin: 0 auto;">
    <h2 style="font-size: 24px; font-weight: 700; margin-bottom: 24px;">Edit Article</h2>
    <form method="POST" action="<?= url(\'/smoronika/<?= $article['id'] ?>/edit\') ?>" enctype="multipart/form-data">
        <?= csrf_field() ?>
        <div style="margin-bottom: 20px;">
            <label style="display: block; font-weight: 600; margin-bottom: 8px;">Title</label>
            <input type="text" name="title" value="<?= htmlspecialchars($article['title']) ?>" required class="w-full px-4 py-3 border-2 rounded-lg">
        </div>
        <div style="margin-bottom: 20px;">
            <label style="display: block; font-weight: 600; margin-bottom: 8px;">Content</label>
            <textarea name="content" rows="15" required class="w-full px-4 py-3 border-2 rounded-lg"><?= htmlspecialchars($article['content']) ?></textarea>
        </div>
        <div style="margin-bottom: 20px;">
